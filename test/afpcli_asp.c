...
size_t my_asp_stream_read(ASP *asp, void *data, const size_t length)
{
  ...
  stored = 0;
  while (stored < length) {
    if ((len = read(asp->socket, (u_int8_t *) data + stored,
...
ssize_t len;
  ...
  while (written < length) {
    if (((len = write(asp->socket, (u_int8_t *) data + written,
...
/* ------------------------------------- 
   OK
*/
void my_asp_tickle(ASP asp, const u_int8_t sid, struct sockaddr_at *sat)
{
...
/* ------------------------------- */
static int  SendCmdWithU16(ASP *asp, int cmd, u_int16_t param)
{
int ofs;
...
/* -------------------------------  */
int AFPCloseVol(ASP *asp, u_int16_t vol)
{
...
/* -------------------------------  */
int AFPCloseDT(ASP *asp, u_int16_t vol)
{
...
/* ------------------------------- */
int AFPCloseFork(ASP *asp, u_int16_t vol)
{
...
/* ------------------------------- */
int AFPFlush(ASP *asp, u_int16_t vol)
{
...
/* -------------------------------  */
int AFPFlushFork(ASP *asp, u_int16_t vol)
{
...
/* -------------------------------  */
u_int16_t AFPOpenVol(ASP *asp, char *vol)
{
int ofs;
u_int16_t bitmap,result, volID = 0;
int len;
...
*/
static void u2mac(char *dst, char *name, int len)
{
	while (len) {
		if (*name == '/') {
			*dst = 0;
...
/* ------------------------- */
int  AFPCreateFile(ASP *asp, u_int16_t vol, char type, int did , char *name)
{
int ofs;
int len;
...
/* ------------------------------- */
int AFPWrite(ASP *asp, u_int16_t fork, int offset, int size, char *data, char whence)
{
int ofs;
int rsize;
...
*/
u_int16_t  AFPOpenFork(ASP *asp, u_int16_t vol, char type, u_int16_t bitmap, int did , char *name,u_int16_t access)
{
int ofs;
int len;
...
/* ------------------------------- */
int AFPDelete(ASP *asp, u_int16_t vol, int did , char *name)
{
int ofs;
u_int16_t bitmap;
int len;
...
/* ------------------------------- */
int AFPRead(ASP *asp, u_int16_t fork, int offset, int size, char *data)
{
int ofs;
int rsize;
...
/* -------------------------------- */
int  AFPCreateDir(ASP *asp, u_int16_t vol, int did , char *name)
{
int ofs;
int len;
int dir = 0;
...
/* ------------------------------- */
u_int16_t  AFPGetForkParam(ASP *asp, u_int16_t fork, u_int16_t bitmap)
{
int ofs;
u_int16_t result;
